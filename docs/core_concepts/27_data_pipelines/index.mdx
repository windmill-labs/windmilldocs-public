---
description: How do I run efficient ETL and data processing in Windmill? DAG-based processing with S3 streaming, Polars and DuckDB, and SQL-to-S3.
...
In essence, an ETL (Extract, Transform, Load) process is a [Directed Acyclic Graph](https://en.wikipedia.org/wiki/Directed_acyclic_graph) (DAG) of jobs where each job reads data, performs computations, and outputs new or updated datasets. This page covers the data-processing side: efficient S3 streaming, Polars and DuckDB patterns, and SQL-to-S3, assembled in the flow editor.

This page focuses on the data-processing side: reading, transforming and writing datasets efficiently, independent of how the steps are orchestrated.

Windmill makes data processing fast, reliable and straightforward to build:

- **Efficient processing**: stream large datasets to and from [object storage](../38_object_storage_in_windmill/index.mdx) and process them with Polars or DuckDB without loading everything into memory (see the recipes and benchmarks below).
...
- [Pipelines](../63_pipelines/index.mdx) (alpha): a lighter, declarative model where independent scripts in a folder are wired automatically by asset lineage, with comment annotations for schedules, partitions, joins and debounce. Best when steps are owned independently and connected only by the datasets they exchange.
