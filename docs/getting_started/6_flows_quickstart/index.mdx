...
On another tab, you can configure a [Schedule](../../core_concepts/1_scheduling/index.mdx) to trigger your flow. Flows can be [triggered](../8_triggers/index.mdx) by any schedules, their [webhooks](../../core_concepts/4_webhooks/index.mdx) or their UI but they only have one primary schedule with which they share the same path. This menu is where you set the primary schedule with CRON. The default schedule is none.
